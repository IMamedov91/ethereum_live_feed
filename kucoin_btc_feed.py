#!/usr/bin/env python3
"""
kucoin_btc_feed.py — v1.1
• 15-min futures-candles (BTCUSDTM) van KuCoin
...
FETCH_LEN       = 550
SNAPSHOT_LEN    = 300
MS_PER_BAR      = TF_DEFAULT_MIN * 60 * 1000
...
def _get(params: dict, retries: int = 3) -> list[list[t.Any]]:
    """ Eenvoudige retry-wrapper rond requests.get(). """
...
def fetch_frame(symbol: str, tf_min: int) -> pd.DataFrame:
    """↩︎ DataFrame met ≥ FETCH_LEN raw candles + indicatoren."""
    raw: list[list[t.Any]] = []
    end_ms = int(time.time() * 1000)
    gran_sec = tf_min * 60                       # 15  → 900
    ...
    cols = ["ts", "open", "close", "high", "low", "vol"]   # futures = 6 cols
    ...
    #  EMA-200 en ATR-14 alleen berekenen als er genoeg regels zijn
    df["ema200"] = (ta.trend.ema_indicator(df["close"], 200)
                    if len(df) >= 200 else float("nan"))
    df["atr14"]  = (ta.volatility.average_true_range(
                        df["high"], df["low"], df["close"], 14)
                    if len(df) >= 14 else float("nan"))
...
# ─────────────────────── utilities ────────────────────────────────
iso    = lambda ms: dt.datetime.utcfromtimestamp(ms/1000)\
...
def main() -> None:
    ...
    df   = fetch_frame(SYMBOL_DEFAULT, TF_DEFAULT_MIN)
    last = df.iloc[-1]
